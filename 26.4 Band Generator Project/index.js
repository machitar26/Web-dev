...
app.get("/", (req, res) => {
  res.render("index.ejs", {
    label: "<h1>Welcome to the Band Generator 🤟</h1>"
  });
});
...
app.post("/submit", (req, res) => {
  ...
  //3. Test to make sre that the random words display in the h1 element in index.ejs

  const randomAdjective = adj[Math.floor(Math.random() * adj.length)];
  const randomNoun = noun[Math.floor(Math.random() * noun.length)];
  res.render("index.ejs", {
    label: `<h1>${randomAdjective} ${randomNoun}</h1>`
  });
});
